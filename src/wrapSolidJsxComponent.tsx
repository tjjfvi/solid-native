...
export function wrapSolidJsxComponent(Comp: any, name = Comp.name, _WrapperReactComp?: any) {
  const _ReactComp = (props: any) => {
    const solidProps = $solidProps in props ? props[$solidProps] : useSolidProps(props);
  ...
  ReactComp[$solidVersion] = (solidProps: any) => (
    <ReactComp key={keyForProps(solidProps)} {...{[$solidProps]: solidProps}} />
  );
  // @ts-ignore
